...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.employees.update", [$employee->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="services_id">{{ trans('cruds.employee.fields.services') }}</label>
                <select class="form-control select2 {{ $errors->has('services') ? 'is-invalid' : '' }}" name="services_id" id="services_id">
                    @foreach($services as $id => $entry)
                        <option value="{{ $id }}" {{ (old('services_id') ? old('services_id') : $employee->services->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
